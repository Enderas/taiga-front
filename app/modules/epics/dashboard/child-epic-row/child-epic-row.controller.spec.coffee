...
#
# File: child-epic-row.controller.spec.coffee
###

describe "ChildEpicRow", ->
    childEpicRowCtrl =  null
    provide = null
    controller = null
    mocks = {}
    ...
    it "calculate progress bar in open US", () ->
        ctrl = controller "ChildEpicRowCtrl", null, {
            childEpic: Immutable.fromJS({
                status_extra_info: {
                    is_closed: false
                }
                user_stories_counts: {
    ...
    it "Update Epic Status Success", (done) ->
        ...
        promise = mocks.tgEpicsService.updateEpicStatus
            .withArgs(ctrl.childEpic, statusId)
            .promise()
            .resolve()
